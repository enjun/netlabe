function [  ] = Final_Traffic( p1, p2 ,p3, p4, l, remove_method, l1, sim_time)
%arrival list fix
if input('1: exponential \n2: poisson\n') == 1 
    %exponential
    m = input('give m ');
    x_0 = input('give seed ');
    a = input('give a ');
    c = input('give c ');
    [Arrival_List] = sort( expo( m, x_0, a, c, l1 ) );
else
    %poisson
    m = input('give m ');
    x_0 = input('give seed ');
    a = input('give a ');
    c = input('give c ');
    [Arrival_List] = sort( poisson( m, x_0, a, c, l1 ) )
end

%size list fix
[Size_List] = sort( uni(m, x_0, c , a) );

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
cutter = 1;
while ( Arrival_List(cutter) <= sim_time)
    cutter = cutter + 1;
    if cutter > length(Arrival_List)
        break;
    end
end

if length( Arrival_List ) > cutter
    Arrival_List( cutter + 1 : length(Arrival_List) ) = [];
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
timer = 0;%event3 time
%disp('event list')
Event_List=[1, 3 ; Arrival_List(1), timer];
...
while flag
    ...
    elseif event == 3 
        disp('event 3')
        [Q, total_pack_removed, Event_List, total_bytes_removed, timer] = Event3(remove_method , Q, total_pack_removed, Event_List, l, total_bytes_removed, timer);
    end
Event_List(:,1)=[];
...
function [Q, total_pack_removed, Event_List, total_bytes_removed, timer] = Event3(remove_method , Q, total_pack_removed, Event_List, l, total_bytes_removed, timer);
s = size(Q);
if s(2) == 0
    disp('no packets to remove')
    timer = timer + 1;
    Event_List(1,end+1)=3;
    Event_List(2,end)= timer;
    return;
end
packs2rem = poissrnd(l); % how many packets to remove
%T=Event_List(2,1);
